...
//Funcion para obtener la hora actual con microsegundos
void horaActual(char *buffer, size_t tam) {
    struct timeval tv; // Estructura para tiempo con microsegundos
    // Campos de struct timeval: tv_sec (segundos), tv_usec (microsegundos)

    struct tm *tm_info; // Estructura para tiempo desglosado
    // Campos: tm_hour, tm_min, tm_sec, tm_mday, tm_mon, tm_year

    gettimeofday(&tv, NULL); // Obtener tiempo actual con microsegundos
    
    tm_info = localtime(&tv.tv_sec); // Convertir de Epoch (segundos desde 1-1-1970) a tiempo local
    
    // Con snprintf escribimos en el buffer el formato deseado
...
int main(){
    ...
    */    
    ...
    // mask: señales a bloquear
    // pending: señales que están pendientes (bloqueadas pero recibidas)
    ...
    sigemptyset(&sa.sa_mask); // Máscara vacía, no se bloquea ninguna señal durante la ejecución del manejador
    sa.sa_flags = SA_RESTART; // Flag para reiniciar llamadas al sistema interrumpidas por señales
    ...
    // Se bloquea antes del fork para que el hijo no pueda enviar la señal antes del bloqueo.
    ...
    if(H1 < 0){
        ...
        if(N1 < 0){
            ...
            kill(padre, SIGUSR2); // N1 envía SIGUSR2 al abuelo (proceso P)
            // Esta señal NO está bloqueada, por lo que se procesará inmediatamente
            // ejecutando el manejador 'gestion'
            ...
            exit(38); //Termina con identificador unico 38
            
        } else {
            /* H1 ESPERA A N1 */
            int statusN; //Variable para almacenar el estado de salida de N1
            horaActual(hora, sizeof(hora));
            printf("[%d] [%s] (H1) Esperando a N1 (PID:%d)\n", hijo, hora, N1);
            
            wait(&statusN); //Bloquea hasta que N1 termine
            //Devuelve el código de salida de N1 
            
            int codigoN = WEXITSTATUS(statusN); //Obtiene el código de salida de N1
        ...
        sleep(10); 
        // El padre duerme 10 segundos para dar tiempo a que:
        // H1 envíe SIGUSR1 (que quedará pendiente)
        // N1 envíe SIGUSR2 (que se procesará inmediatamente) y termine
        ...
        // Con sigismember se comprueba si una señal está en el conjunto de pendientes obtenido con sigpending
        ...
        // SIG_UNBLOCK: elimina las señales de 'mask' del conjunto de bloqueadas
        // Al desbloquear, la señal pendiente se entrega inmediatamente
